...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        const fetchTimetable = async () => {
            try {
                // We need student profile to get course/year/sem
                // Assuming user object has profile info or we fetch it
                // Let's fetch profile first if not in user object
                const profileRes = await api.get(`students/?user=${user.id}`);
                if (profileRes.data.length > 0) {
                    const profile = profileRes.data[0];
                    const response = await api.get(`timetable/?course_id=${profile.course}&year=${profile.year}&semester=${profile.semester}`);
    ...
    return (
        <div className="p-6">
            ...
            <div className="bg-white p-6 rounded shadow overflow-x-auto">
                <h2 className="text-xl font-semibold mb-4">My Class Schedule</h2>
                    ...
                    <tbody>
                        {timetable.map((slot) => (
                            <tr key={slot.id} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-2">{slot.subject_details.lecturer_name}</td>
                            </tr>
                        ))}
                        {timetable.length === 0 && (
